      ...
      <WebView
        ...
        onLoadProgress={event => {
          ...
          if (
            (actualEvent.title &&
              Platform.OS == 'android' &&
              actualEvent.title.indexOf('sharewheelz.in/api/auth/paysuccess') >=
                0) ||
            (actualEvent.url &&
              Platform.OS == 'ios' &&
              actualEvent.url.indexOf('sharewheelz.in/api/auth/paysuccess') >=
                0)
          ) {
            console.log('success event ', actualEvent.url);
            navigation.navigate('PaymentSuccess', {data: actualEvent.url});
          } else if (
            (actualEvent.title &&
              Platform.OS == 'android' &&
              actualEvent.title.indexOf('sharewheelz.in/api/auth/payfailed') >=
                0) ||
            (actualEvent.url &&
              Platform.OS == 'ios' &&
              actualEvent.url.indexOf('sharewheelz.in/api/auth/payfailed') >= 0)
          ) {
            console.log('success event');
            navigation.navigate('PaymentFailure', {data: actualEvent.url});
          }
