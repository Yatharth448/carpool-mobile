...
import {
  cameraPermission,
  galleryPermission,
} from '../../Utils/RuntimePermission';
import selectImage from '../../Utils/ImagePicker';
import {
  hitApiForDLKYC,
  hitApiForUploadDocument,
...
export default function UploadDocuments({navigation, route}) {
  ...
  const AadharCardKYC = async () => {
    ...
    navigation.navigate('VerifyAadharOTP', {
      clientId: result.clientId,
      name: aadharName,
      aadharNumber: docNumber,
    });
  };
  ...
  const openCam = async () => {
    try {
      if (await cameraPermission()) {
        ...
        imageName =
          imageName.length > 20
            ? `${String(imageName).substring(0, 18)}...` + extension
            : imageName;
  ...
  const openGall = async () => {
    ...
    try {
      if (await galleryPermission()) {
        ...
        setAadharName('');
        setDocNumber('');
        setSelectedDate('DOB as per Driving Licence');
        setLicenceName('');
        // }
        // else {
  ...
  return (
    <ScrollView>
      <View
            ...
            <View
              ...
              <View
                ...
                <Image
                  source={
                    data.index == 0
                      ? require('../../assets/aadhar.png')
                      : data.index == 1
                      ? require('../../assets/driverslicense.png')
          ...
          <View style={{width: '100%', alignItems: 'center', marginTop: 20}}>
            <View
              ...
              <Pressable onPress={() => uploadImagePress('back')}>
                <Image
                  source={
                    bckImg ? {uri: bckImg} : require('../../assets/backdoc.png')
                  }
                  style={{
                    width: screenWidth,
                    height: screenWidth,
        ...
        <CommonLoaders.ChatLoader
          isLoading={isLoading}
          loaderText={'Updating KYC...Please wait'}
        />
      </View>
    </ScrollView>
  );
