...
export default function PaymentSuccess({ navigation, route }) {
    ...
    return (
        <View style={{ flex: 1, width: '100%', backgroundColor: AppColors.themePickupDropSearchBg, alignItems: 'center' }}>
            <Header isBack={true} close={() => navigation.pop(2)} text="Success" />
            <View style={{ width: '100%', justifyContent: 'center', alignItems: 'center', backgroundColor: AppColors.themesWhiteColor }}>

                <Image source={require('../../assets/success.gif')} style={{ width: Dimensions.get('screen').width, height: 200, resizeMode: 'contain' }} />

                <Text style={{ marginTop: 5, fontSize: 20, fontFamily: AppFontFamily.PopinsSemiBold, color: AppColors.themeGreenColor }}>{"Transaction Successful"}</Text>
            </View>
            <View>
